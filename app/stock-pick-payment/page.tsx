...
export default function PaymentPage() {
  ...
  const stockPrice = Number(searchParams.get("price"));
  ...
  const downloadReceipt = () => {
    const receiptContent = `
      ...
      Your stock symbol will be send directly to your email.
    `
    ...
    onSubmit: async (values, { resetForm, setSubmitting }) => {
      try {
        ...
        formattedData = {
          ...
          payment_amount: stockPrice,
          payment_reference: payment_slip_url.name,
          payment_notes: stockId + payment_slip_url.name
        };
        const res = await postAPI({
          url: `/stock-picks/selections/${stockId}/payment-slip`,
          body: formattedData,
        });
                  ...
                  {paymentMethod === "qr" && (
                    <div className="text-center p-6 bg-muted/30 rounded-lg">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Total amount: <span className="font-semibold text-primary">${stockPrice}</span>
                      </p>
                    </div>
                  )}
                    ...
                    <div className="p-6 bg-muted/30 rounded-lg">
                      ...
                      <div className="space-y-3 text-sm">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="font-semibold text-primary">${stockPrice}</span>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="flex items-center justify-center w-full gap-4">
                    <div className="flex items-center jsutify-center gap-2 w-full sm:w-1/2">
                      <Button variant="outline" asChild className="w-auto flex-1 hover:border hover:border-primary hover:bg-transparent">
                        <Link href="/stock-picks"><ArrowLeft className="w-4 h-4" />&nbsp;Pick more</Link>
                      </Button>
